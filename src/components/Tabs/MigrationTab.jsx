...
export const MigrationTab = ({ showToast }) => {
  ...
  // Available Bridge OS Categories
  const [bridgeCategories, setBridgeCategories] = useState([
  ...
  // Field Mappings (Bridge Field -> CSV Header Index)
  ...
  // Mappings of legacy Category -> Bridge Category
    ...
    apiFetch('/categories')
      .then(cats => {
        if (Array.isArray(cats) && cats.length > 0) {
          ...
          setBridgeCategories(prev => Array.from(new Set([...cats, ...prev])).sort());
        }
      })
      .catch(() => {});
  }, []);
  ...
  const prepareCategoryMappingStep = () => {
    ...
    sortedCats.forEach(lc => {
      // Try to guess match
      const matched = bridgeCategories.find(vc => 
  ...
  const handleCategoryMapChange = (legacyCat, bridgeCat) => {
    setCategoryMappings(prev => ({ ...prev, [legacyCat]: bridgeCat }));
  };
        ...
        {step === 4 && (
          <div className="space-y-8 text-center py-8">
            ...
            <div className="flex justify-center gap-4 pt-6">
              <a
                href={`${window.bridgeData?.site_url || '/'}wp-admin/admin.php?page=pmxi-admin-import`}
