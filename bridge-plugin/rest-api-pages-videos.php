<?php
/**
 * Bridge OS — REST API Handlers: Pages & Video management.
 *
 ...
 * All dependencies: WordPress globals (no bridge-backend.php dependency).
 */
...
function bridge_extract_youtube_url(string $embed_html): string {
...
function bridge_get_youtube_embed_html(string $url): string {
...
function bridge_save_video_fields(int $post_id, string $youtube_url, int $category_id, string $video_file_url = '', int $video_file_id = 0): void {
    $embed_html = bridge_get_youtube_embed_html($youtube_url);
...
function bridge_api_get_videos(): WP_REST_Response {
    ...
    foreach ($posts as $p) {
        ...
        $videos[] = array(
            'id'            => $p->ID,
            'title'         => $p->post_title,
            'youtube_link'  => bridge_extract_youtube_url(
...
function bridge_api_create_video(WP_REST_Request $request) {
    ...
    bridge_save_video_fields($post_id, $youtube_link, $category_id, $video_file_url, $video_file_id);

    return rest_ensure_response(array('success' => true, 'id' => $post_id));
}

function bridge_api_update_video(WP_REST_Request $request) {
...
function bridge_api_delete_video(WP_REST_Request $request) {
    $post_id = intval($request->get_param('id'));
    if (!get_post($post_id)) {
        return new WP_Error('not_found', 'Video not found.', array('status' => 404));
    }
...
function bridge_api_get_video_categories() {
...
function bridge_api_create_video_category(WP_REST_Request $request) {
    $data = $request->get_json_params();
...
function bridge_api_delete_video_category(WP_REST_Request $request) {
...
function bridge_api_list_pages(): WP_REST_Response {
 ...
 */
function bridge_is_protected_page(int $id): bool {
    if ($id <= 0) {
        return false;
    }
    ...
    $post = get_post($id);
    if ($post && (has_shortcode($post->post_content, 'bridge_showroom') || strpos($post->post_content, '[bridge_showroom]') !== false)) {
        return true;
    }
 ...
 */
function bridge_is_valid_page_template(string $template): bool {
...
function bridge_api_create_page(WP_REST_Request $request): WP_REST_Response|WP_Error {
    ...
    if (!bridge_is_valid_page_template($template)) {
...
function bridge_api_update_page(WP_REST_Request $request): WP_REST_Response|WP_Error {
    ...
    if (bridge_is_protected_page($id)) {
        if ($status === 'draft') {
...
function bridge_api_delete_page(WP_REST_Request $request): WP_REST_Response|WP_Error {
    $id = intval($request->get_param('id'));
    $post = get_post($id);
    if (!$post || $post->post_type !== 'page') {
        return new WP_Error('not_found', 'Page not found.', array('status' => 404));
...
function bridge_api_get_page_templates(): WP_REST_Response {
    $theme = wp_get_theme();
    $templates = $theme->get_page_templates();
    $out = array();
