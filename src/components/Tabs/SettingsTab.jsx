...
export const SettingsTab = ({ showToast }) => {
  ...
  const SECTION_ANCHORS = {
    ...
    contact: 'bridge-map',       // contact/map is in footer.php
    hours:   'bridge-map',       // same section — hours live near the map
    about:   'section-cta',
  ...
  useEffect(() => {
    if (!isLoading) {
      ...
      const baseUrl = window.bridgeData?.site_url || '/';
      setPreviewUrl(`${baseUrl}?bridge_preview=1`);
    }
  }, [isLoading]);
  ...
  useEffect(() => {
    ...
    const timer = setTimeout(async () => {
      try {
        ...
        iframeReady.current = false;
        const baseUrl = window.bridgeData?.site_url || '/';
        setPreviewUrl(`${baseUrl}?bridge_preview=1&t=${Date.now()}`);
      } catch (err) {
        ...
        <div className="flex-1 bg-slate-950 relative overflow-hidden">
          {previewUrl ? (
            <iframe 
              ...
              title="Bridge Equipment Site Preview"
              onLoad={() => {
