 ...
 * Depends on bridge_schedule_catalog_regeneration() in bridge-facebook-pwa.php.
 */
...
function bridge_log_meta_sync(string $message, string $type = 'success'): void {
...
add_action('acf/save_post', 'bridge_acf_save_meta_sync_log', 20);
function bridge_acf_save_meta_sync_log($post_id): void {
    ...
    if ($prev_data) {
        ...
        if ($prev_sync !== $facebook_sync) {
            if ($facebook_sync) {
                bridge_log_meta_sync("Inventory Synced: {$display_name}");
            } else {
                bridge_log_meta_sync("Inventory Unsynced: {$display_name}", 'warning');
            }
        } elseif ($prev_data['price'] != $current_data['price']) {
            bridge_log_meta_sync("Price Sync: {$display_name}");
        } elseif (wp_json_encode($prev_data['gallery']) != wp_json_encode($current_data['gallery'])) {
            bridge_log_meta_sync("New Media: {$display_name}");
        } else {
            bridge_log_meta_sync("Inventory Update: {$display_name}");
        }
    } else {
        if (!$facebook_sync) {
            bridge_log_meta_sync("Inventory Unsynced: {$display_name}", 'warning');
...
add_action('wp_trash_post', 'bridge_trash_meta_sync_log');
function bridge_trash_meta_sync_log($post_id): void {
    if (get_post_type($post_id) !== 'equipment') {
        return;
    }
    $title = get_the_title($post_id);
    ...
    bridge_log_meta_sync("Inventory Removed: {$display_name}", 'warning');
...
add_action('untrash_post', 'bridge_untrash_meta_sync_log');
function bridge_untrash_meta_sync_log($post_id): void {
    if (get_post_type($post_id) !== 'equipment') {
        return;
    }
    $title = get_the_title($post_id);
    ...
    bridge_log_meta_sync("Inventory Restored: {$display_name}");
...
add_action('before_delete_post', 'bridge_delete_meta_sync_log');
function bridge_delete_meta_sync_log($post_id): void {
    if (get_post_type($post_id) !== 'equipment') {
        return;
    }
    $title = get_the_title($post_id);
    ...
    bridge_log_meta_sync("Inventory Removed (Permanent): {$display_name}", 'warning');
...
add_action('pmxi_after_xml_import', 'bridge_wpaip_import_complete');
function bridge_wpaip_import_complete($import_id): void {
    if (function_exists('bridge_schedule_catalog_regeneration')) {
        bridge_schedule_catalog_regeneration(true);
    }
}
