<?php
/**
 * Bridge OS — Facebook Catalog CSV + PWA Mobile Companion Router
 *
 * Extracted from bridge-os-plugin-v23.php. Loaded via require_once in that file.
 * No dependencies on bridge-backend.php — uses get_post_meta/get_field directly.
...
function bridge_get_facebook_catalog_csv(): string {
    ...
    foreach ($posts as $post) {
        ...
        if (empty($make)) $make = 'Bridge Equipment';
...
function bridge_schedule_catalog_regeneration(bool $force = false): void {
    if ($force) {
        delete_option('bridge_catalog_regen_lock_expiry');
        delete_option('bridge_catalog_dirty');
    }

    $lock_expiry = (int)get_option('bridge_catalog_regen_lock_expiry', 0);
    ...
    if ($force || !defined('WP_IMPORTING') || !WP_IMPORTING) {
        if (!$force && $is_locked) {
            ...
            update_option('bridge_catalog_dirty', 1, false);

            // Rate-limited synchronously; schedule a background job as a fallback
            if (!wp_next_scheduled('bridge_cron_regenerate_catalog')) {
                wp_schedule_single_event(time() + 60, 'bridge_cron_regenerate_catalog');
            }
            return;
        }
        ...
        if (!$force) {
            update_option('bridge_catalog_regen_lock_expiry', time() + 10, false);
        }
        delete_option('bridge_catalog_dirty');
...
add_action('init', 'bridge_maybe_cleanup_dirty_catalog');
function bridge_maybe_cleanup_dirty_catalog(): void {
    ...
    $is_dirty    = (int)get_option('bridge_catalog_dirty', 0) === 1;
    $lock_expiry = (int)get_option('bridge_catalog_regen_lock_expiry', 0);
    $is_locked   = (time() < $lock_expiry);
...
add_action('bridge_cron_regenerate_catalog', 'bridge_cron_regenerate_catalog');
function bridge_cron_regenerate_catalog(): void {
    delete_option('bridge_catalog_dirty');
    bridge_write_facebook_catalog_file();
}

function bridge_write_facebook_catalog_file(): bool {
    $csv_data = bridge_get_facebook_catalog_csv();
    $upload_dir = wp_upload_dir();
    $file_path = trailingslashit($upload_dir['basedir']) . 'facebook-catalog.csv';
    $written = file_put_contents($file_path, $csv_data);
    if ($written === false) {
        bridge_log_meta_sync("ERROR: Failed to write catalog CSV to {$file_path}", 'warning');
    }
    
    // Clear the meta sync health cache transient so it recalculates next time
    delete_transient('bridge_meta_sync_health');
    
    return $written !== false;
}

function bridge_generate_facebook_catalog(): void {
    status_header(200);
    ...
    if ($is_facebook) {
        bridge_log_meta_sync("API Handshake: Success (Meta crawler synced {$count} items)");
    } else {
        bridge_log_meta_sync("Inventory Update checked (Manual pull: {$count} items synced)");
    }
...
add_action('template_redirect', 'bridge_facebook_catalog_endpoint');
function bridge_facebook_catalog_endpoint(): void {
    $uri      = $_SERVER['REQUEST_URI'] ?? '';
    ...
    if ($path === 'facebook-catalog.csv') {
        bridge_generate_facebook_catalog();
    }
}

// ─── PWA / Mobile Companion Router ────────────────────────────────────────────────

add_action('init', 'bridge_mobile_pwa_router');
function bridge_mobile_pwa_router(): void {
    $uri      = $_SERVER['REQUEST_URI'] ?? '';
    $uri_path = trim(parse_url($uri, PHP_URL_PATH), '/');
    ...
    if ($path === 'mobile-app/manifest.json' || $path === 'manifest.json') {
        $icon_cache_key = 'bridge_pwa_icon_url';
        ...
        if (!$icon_url) {
            if (file_exists(get_stylesheet_directory() . '/assets/VE_Tractor_Icon.png')) {
                ...
                $icon_url   = file_exists($upload_dir['basedir'] . '/2026/04/VE_Tractor_Icon.png')
                    ...
                    : (bridge_get_brand_logo_url('red') ?: plugin_dir_url(__FILE__) . 'dist/assets/logo.png');
            }
        ...
        echo json_encode(array(
            'id'               => '/mobile-app/',
            'name'             => 'Bridge OS Mobile',
            'short_name'       => 'Bridge OS',
            'description'      => 'Mobile inventory management for Bridge Equipment yard crew.',
    ...
    if ($path === 'mobile-app/sw.js' || $path === 'sw.js') {
        ...
        if (file_exists($html_file)) {
            $sw_cache_key = 'bridge_assets_' . filemtime($html_file);
        ...
        $sw_icon_url = esc_url_raw(get_transient('bridge_pwa_icon_url') ?: '');
        ?>
/**
 * Bridge OS — Mobile Companion Service Worker
...
const CACHE_NAME    = 'bridge-os-' + CACHE_VERSION;
...
const NETWORK_ONLY = ['/wp-json/', '/bridge/v1', '/wp-admin/', '/wp-login.php', '/sw.js', '/manifest.json'];
...
self.addEventListener('install', (event) => {
    event.waitUntil(
        caches.open(CACHE_NAME)
            .then((cache) => {
                console.log('[Bridge SW] Pre-caching ' + PRE_CACHE.length + ' assets (v' + CACHE_VERSION + ')');
    ...
    event.waitUntil(
        caches.keys()
            .then((keys) => {
                ...
                if (stale.length) console.log('[Bridge SW] Purging ' + stale.length + ' stale cache(s)');
    ...
    if ($path === 'mobile-app') {
        ...
        if (!empty($_GET['handoff'])) {
            ...
            if (strlen($handoff_nonce) === 32 && ctype_xdigit($handoff_nonce)) {
                $has_auth_param = (bool) get_transient('bridge_handoff_' . $handoff_nonce);
            }
        }
        ...
        if (!$has_auth_param) {
            ...
            if (!current_user_can('edit_posts')) {
                ...
                echo '<!DOCTYPE html><meta charset="utf-8"><title>Not authorized</title>'
                   ...
                   . '<p>This account can\'t use the Bridge OS app. Ask an admin to set your role to <strong>Editor</strong>.</p>'
...
<head>
    ...
    <meta name="apple-mobile-web-app-title" content="Bridge OS">
    ...
    <style>
        ...
        #bridge-inventory-app { width:100%; height:100%; overflow:hidden; }
    </style>
    <script>
    ...
    <?php
    // Use the same transient cache as bridge_enqueue_react_assets()
    $html_file = plugin_dir_path(__FILE__) . 'dist/index.html';
    $js_file = $css_file = '';
    if (file_exists($html_file)) {
...
if (!empty($_GET['handoff'])) {
    $handoff_nonce = sanitize_text_field(wp_unslash($_GET['handoff']));
    if (strlen($handoff_nonce) === 32 && ctype_xdigit($handoff_nonce)) {
        $resolved = get_transient('bridge_handoff_' . $handoff_nonce);
        if ($resolved) {
            $mobile_token_for_page = $resolved;
            delete_transient('bridge_handoff_' . $handoff_nonce); // one-time use
        }
    }
}
...
if ($mobile_token_for_page) {
    bridge_set_mobile_token_cookie($mobile_token_for_page);
}
?>
window.bridgeData = {
    post_id: 0,
    ...
    is_mobile_app: true,
    logo_url: '<?php echo function_exists('bridge_get_brand_logo_url') ? esc_url(bridge_get_brand_logo_url('white')) : ''; ?>'
};
</script>
...
<body>
    <div id="bridge-inventory-app" class="bridge-inventory-app-mount"></div>
</body>
</html>
        <?php
        exit;
