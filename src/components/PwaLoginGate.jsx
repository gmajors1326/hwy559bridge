...
export const PwaLoginGate = ({ mobileToken, setMobileToken, toast, onAuthenticated }) => {
  ...
  useEffect(() => {
    ...
    window.addEventListener('bridge:token-expired', onTokenExpired);
    return () => window.removeEventListener('bridge:token-expired', onTokenExpired);
  }, [setMobileToken]);
  ...
  const handleCredentialLogin = async (e) => {
    ...
    setAuthError('');
    try {
      const restUrl = window.bridgeData?.rest_url || '/wp-json/';
      const res = await fetch(`${restUrl}bridge/v1/login`, {
        method: 'POST',
      ...
      <div className="flex-1 flex flex-col justify-center items-center px-6 py-12">
        <div className="w-full max-w-sm space-y-8">
          <div className="text-center">
            ...
            <h1 className="text-3xl font-black tracking-tighter uppercase text-white">Bridge OS</h1>
