<?php
/**
 * Bridge OS — REST API Routes & Handlers
 *
 * All /bridge/v1/* endpoints, video management, and settings API.
 * Auth filters and session tracking live in bridge-os-plugin-v23.php.
 */
 ...
 */
function bridge_is_admin_request(): bool {
...
function bridge_register_rest_routes(): void {
    add_filter('rest_post_dispatch', function($response, $server, $request) {
        if ($response instanceof WP_REST_Response && str_contains($request->get_route(), '/bridge/v1')) {
    ...
    register_rest_route($ns, '/inventory', array(
        array(
            'methods'             => 'GET',
            'callback'            => 'bridge_api_get_inventory',
        ...
        array(
            'methods'             => 'POST',
            'callback'            => 'bridge_api_create_unit',
            'permission_callback' => $auth,
        ),
    ));
    register_rest_route($ns, '/inventory/deleted', array(
        'methods'             => 'GET',
        'callback'            => 'bridge_api_get_deleted',
    ...
    register_rest_route($ns, '/inventory/(?P<id>\d+)', array(
        array(
            'methods'             => 'GET',
            'callback'            => 'bridge_api_get_unit',
            'permission_callback' => function (): bool { return current_user_can('edit_others_posts'); },
        ),
        array(
            'methods'             => 'PATCH',
            'callback'            => 'bridge_api_update_unit',
            'permission_callback' => $auth,
        ),
        array(
            'methods'             => 'DELETE',
            'callback'            => 'bridge_api_soft_delete',
            'permission_callback' => $auth,
        ),
    ));
    register_rest_route($ns, '/inventory/(?P<id>\d+)/restore', array(
        'methods'             => 'POST',
        'callback'            => 'bridge_api_restore_unit',
        'permission_callback' => $auth,
    ));
    register_rest_route($ns, '/inventory/(?P<id>\d+)/marketplace-posted', array(
        'methods'             => 'POST',
        'callback'            => 'bridge_api_set_marketplace_posted',
        'permission_callback' => $auth,
    ));
    register_rest_route($ns, '/inventory/(?P<id>\d+)/permanent', array(
        'methods'             => 'DELETE',
        'callback'            => 'bridge_api_permanent_delete',
        'permission_callback' => function (): bool { return current_user_can('edit_others_posts'); },
    ));
    register_rest_route($ns, '/inventory/(?P<id>\d+)/ledger', array(
        'methods'             => 'GET',
        'callback'            => 'bridge_api_get_ledger',
        'permission_callback' => function (): bool { return current_user_can('edit_others_posts'); },
    ));

    register_rest_route($ns, '/media', array(
        'methods'             => 'POST',
        'callback'            => 'bridge_api_upload_media',
        'permission_callback' => $auth,
    ));

    register_rest_route($ns, '/brands', array(
        array('methods' => 'GET',  'callback' => 'bridge_api_get_brands',  'permission_callback' => $auth),
        array('methods' => 'POST', 'callback' => 'bridge_api_save_brands', 'permission_callback' => $auth),
    ));
    register_rest_route($ns, '/years', array(
        array('methods' => 'GET',  'callback' => 'bridge_api_get_years',  'permission_callback' => $auth),
        array('methods' => 'POST', 'callback' => 'bridge_api_save_years', 'permission_callback' => $auth),
    ));
    register_rest_route($ns, '/categories', array(
        array('methods' => 'GET',  'callback' => 'bridge_api_get_categories',  'permission_callback' => $auth),
    ));
    register_rest_route($ns, '/subcategories', array(
        array('methods' => 'GET',  'callback' => 'bridge_api_get_subcategories',  'permission_callback' => $auth),
    ));
    register_rest_route($ns, '/sub-subcategories', array(
        array('methods' => 'GET',  'callback' => 'bridge_api_get_sub_subcategories',  'permission_callback' => $auth),
    ));
    register_rest_route($ns, '/category-tree', array(
        array('methods' => 'GET',  'callback' => 'bridge_api_get_category_tree',  'permission_callback' => $auth),
        array('methods' => 'POST', 'callback' => 'bridge_api_save_category_tree', 'permission_callback' => $auth),
    ));
    register_rest_route($ns, '/category-tree/rename', array(
        array('methods' => 'POST', 'callback' => 'bridge_api_rename_category_node', 'permission_callback' => $auth),
    ));
    register_rest_route($ns, '/category-tree/node', array(
        array('methods' => 'POST',   'callback' => 'bridge_api_category_tree_add',    'permission_callback' => $auth),
        array('methods' => 'DELETE', 'callback' => 'bridge_api_category_tree_delete', 'permission_callback' => $auth),
    ));

    register_rest_route($ns, '/videos', array(
        array('methods' => 'GET',  'callback' => 'bridge_api_get_videos',  'permission_callback' => $auth),
        array('methods' => 'POST', 'callback' => 'bridge_api_create_video', 'permission_callback' => $auth),
    ));
    register_rest_route($ns, '/videos/(?P<id>\d+)', array(
        array('methods' => 'PATCH',  'callback' => 'bridge_api_update_video', 'permission_callback' => $auth),
        array('methods' => 'DELETE', 'callback' => 'bridge_api_delete_video', 'permission_callback' => $auth),
    ));
    register_rest_route($ns, '/video-categories', array(
        array('methods' => 'GET',  'callback' => 'bridge_api_get_video_categories',  'permission_callback' => $auth),
        array('methods' => 'POST', 'callback' => 'bridge_api_create_video_category', 'permission_callback' => $auth),
    ));
    register_rest_route($ns, '/video-categories/(?P<id>\d+)', array(
        array('methods' => 'DELETE', 'callback' => 'bridge_api_delete_video_category', 'permission_callback' => $auth),
    ));

    register_rest_route($ns, '/sessions', array(
        'methods'             => 'GET',
        'callback'            => 'bridge_api_get_sessions',
    ...
    register_rest_route($ns, '/ledger', array(
        'methods'             => 'GET',
        'callback'            => 'bridge_api_get_global_ledger',
        'permission_callback' => function (): bool { return current_user_can('manage_options'); },
        'args'                => array(
            'page'     => array('type' => 'integer', 'minimum' => 1, 'default' => 1,  'sanitize_callback' => 'absint'),
            'per_page' => array('type' => 'integer', 'minimum' => 1, 'maximum' => 100, 'default' => 20, 'sanitize_callback' => 'absint'),
    ...
    register_rest_route($ns, '/session', array(
        'methods'             => 'GET',
        'callback'            => 'bridge_api_session_nonce',
        'permission_callback' => '__return_true',
    ));
    ...
    register_rest_route($ns, '/bootstrap', array(
        'methods'             => 'GET',
        'callback'            => 'bridge_api_bootstrap',
        'permission_callback' => $auth,
    ));
    register_rest_route($ns, '/logout', array(
        'methods'             => 'POST',
        'callback'            => 'bridge_api_logout',
        'permission_callback' => 'is_user_logged_in',
    ));
    register_rest_route($ns, '/mobile/token', array(
        'methods'             => 'POST',
        'callback'            => 'bridge_api_generate_mobile_token',
        'permission_callback' => $auth,
    ));
    register_rest_route($ns, '/login', array(
        'methods'             => 'POST',
        'callback'            => 'bridge_api_mobile_login',
    ...
    register_rest_route($ns, '/settings', array(
        array(
            'methods'             => 'GET',
            'callback'            => 'bridge_api_get_settings',
            'permission_callback' => function (): bool { return current_user_can('manage_options'); },
        ),
        array(
            'methods'             => 'POST',
            'callback'            => 'bridge_api_save_settings',
            'permission_callback' => function (): bool { return current_user_can('manage_options'); },
        ),
    ));
    register_rest_route($ns, '/settings/preview', array(
        'methods'             => 'POST',
        'callback'            => 'bridge_api_save_preview_settings',
        'permission_callback' => function (): bool { return current_user_can('manage_options'); },
    ));

    register_rest_route($ns, '/meta-sync/logs', array(
        'methods'             => 'GET',
        'callback'            => 'bridge_api_get_meta_sync_logs',
        'permission_callback' => function (): bool { return current_user_can('manage_options'); },
    ));

    register_rest_route($ns, '/meta-sync/health', array(
        'methods'             => 'GET',
        'callback'            => 'bridge_api_get_meta_sync_health',
        'permission_callback' => function (): bool { return current_user_can('manage_options'); },
    ));
    ...
    register_rest_route($ns, '/staff', array(
        array(
            'methods'             => 'GET',
            'callback'            => 'bridge_api_list_staff',
            'permission_callback' => $admin_auth,
        ),
        array(
            'methods'             => 'POST',
            'callback'            => 'bridge_api_create_staff',
            'permission_callback' => $admin_auth,
            'args'                => array(
    ...
    register_rest_route($ns, '/staff/(?P<id>\d+)', array(
        'methods'             => 'DELETE',
        'callback'            => 'bridge_api_delete_staff',
        'permission_callback' => $admin_auth,
        'args'                => array(
    ...
    register_rest_route($ns, '/pages', array(
        array(
            'methods'             => 'GET',
            'callback'            => 'bridge_api_list_pages',
            'permission_callback' => $page_read_auth,
        ),
        array(
            'methods'             => 'POST',
            'callback'            => 'bridge_api_create_page',
    ...
    register_rest_route($ns, '/pages/(?P<id>\d+)', array(
        array(
            'methods'             => 'PATCH',
            'callback'            => 'bridge_api_update_page',
        ...
        array(
            'methods'             => 'DELETE',
            'callback'            => 'bridge_api_delete_page',
            'permission_callback' => function (WP_REST_Request $request): bool {
    ...
    register_rest_route($ns, '/page-templates', array(
        'methods'             => 'GET',
        'callback'            => 'bridge_api_get_page_templates',
        'permission_callback' => $page_read_auth,
    ));
}
...
function bridge_api_list_staff(): WP_REST_Response {
...
function bridge_api_create_staff(WP_REST_Request $req): WP_REST_Response|WP_Error {
...
function bridge_api_delete_staff(WP_REST_Request $req): WP_REST_Response|WP_Error {
...
function bridge_api_get_brands(): WP_REST_Response {
    $brands = get_option('bridge_brands');
    if (!is_array($brands) || empty($brands)) {
        $brands = bridge_default_brands();
    }
    return rest_ensure_response($brands);
}

function bridge_api_save_list(string $param, string $option, WP_REST_Request $request): WP_REST_Response {
...
function bridge_api_save_brands(WP_REST_Request $r): WP_REST_Response {
    delete_transient('bridge_brand_counts');
    return bridge_api_save_list('brands', 'bridge_brands', $r);
}

function bridge_default_years(): array {
    $years = array();
...
function bridge_api_get_years(): WP_REST_Response {
    $years = get_option('bridge_years');
    if (!is_array($years) || empty($years)) {
        $years = bridge_default_years();
    }
    return rest_ensure_response($years);
}

function bridge_api_save_years(WP_REST_Request $r): WP_REST_Response {
    return bridge_api_save_list('years', 'bridge_years', $r);
}

function bridge_default_categories(): array {
    return array(
...
function bridge_api_get_categories(): WP_REST_Response {
    $categories = get_option('bridge_categories');
    if (!is_array($categories) || empty($categories)) {
        $categories = bridge_default_categories();
    }
    return rest_ensure_response($categories);
}

function bridge_api_get_subcategories(): WP_REST_Response {
    $items = get_option('bridge_subcategories');
    return rest_ensure_response(is_array($items) ? $items : array());
}

function bridge_api_get_sub_subcategories(): WP_REST_Response {
    $items = get_option('bridge_sub_subcategories');
    return rest_ensure_response(is_array($items) ? $items : array());
}

function bridge_api_get_category_tree(): WP_REST_Response {
    $tree = get_option('bridge_category_tree', array());
    return rest_ensure_response(is_array($tree) ? $tree : array());
}

function bridge_api_save_category_tree(WP_REST_Request $r): WP_REST_Response|WP_Error {
    ...
    if (function_exists('bridge_derive_flat_options_from_tree')) {
        bridge_derive_flat_options_from_tree();
    }
    return rest_ensure_response($tree);
}

function bridge_api_rename_category_node(WP_REST_Request $r): WP_REST_Response|WP_Error {
    ...
    // Make/brand renames are handled inline (they live in bridge_brands, not the tree)
    if ($type === 'make' || $type === 'brand') {
        $brands = get_option('bridge_brands', array());
        if (!is_array($brands)) {
        ...
        $brands[$idx] = $new;
        update_option('bridge_brands', $brands);
        $affected = bridge_migrate_equipment_meta_scoped(array('make' => $old), 'make', $new);
        return rest_ensure_response(array(
            'success'        => true,
            'category_tree'  => get_option('bridge_category_tree', array()),
            'brands'         => get_option('bridge_brands', array()),
    ...
    $result = bridge_category_tree_mutate('rename', array(
        'type'              => $type,
        'name'              => $old,
        'new_name'          => $new,
...
function bridge_api_category_tree_add(WP_REST_Request $r): WP_REST_Response|WP_Error {
    $result = bridge_category_tree_mutate('add', array(
        'type'               => sanitize_text_field($r->get_param('type') ?? ''),
...
function bridge_api_category_tree_delete(WP_REST_Request $r): WP_REST_Response|WP_Error {
    $result = bridge_category_tree_mutate('delete', array(
        'type'               => sanitize_text_field($r->get_param('type') ?? ''),
        'name'               => sanitize_text_field($r->get_param('name') ?? ''),
        'parent_category'    => sanitize_text_field($r->get_param('parent_category') ?? ''),
        'parent_subcategory' => sanitize_text_field($r->get_param('parent_subcategory') ?? ''),
...
function bridge_api_validate_equipment($post_id) {
...
function bridge_api_get_inventory(WP_REST_Request $request) {
    ...
    $args = array(
        'post_type'      => 'equipment',
        'post_status'    => bridge_is_admin_request() ? array('publish', 'draft') : 'publish',
        'orderby'        => 'date',
        'order'          => 'DESC',
    );

    if (!bridge_is_admin_request()) {
    ...
    $items = array_map(function (WP_Post $p): array {
        return bridge_format_unit($p->ID, current_user_can('edit_others_posts') ? 'edit' : 'public');
    }, $query->posts);
...
function bridge_api_get_deleted(WP_REST_Request $request): WP_REST_Response {
    $raw_per_page = intval($request->get_param('per_page') ?: -1);
    ...
    $query = new WP_Query($args);
    $items = array_map(function (WP_Post $p): array {
        return bridge_format_unit($p->ID);
    }, $query->posts);
...
function bridge_check_duplicate_vin_or_stock(array $data, int $exclude_post_id = 0) {
    global $wpdb;
...
function bridge_api_create_unit(WP_REST_Request $request) {
    $data    = $request->get_json_params();
    if (!is_array($data)) {
        return new WP_Error('invalid_body', 'Invalid JSON body', array('status' => 400));
    }
    
    $dup_error = bridge_check_duplicate_vin_or_stock($data, 0);
    ...
    bridge_save_unit_fields($post_id, $data);
    $unit = bridge_format_unit($post_id);

    $rid = bridge_request_id($request);
    bridge_log_ledger($post_id, 'create', 'created unit', array('after' => $unit), $rid);

    return rest_ensure_response($unit);
}

function bridge_api_update_unit(WP_REST_Request $request) {
    ...
    $dup_error = bridge_check_duplicate_vin_or_stock($data, $post_id);
    if (is_wp_error($dup_error)) {
        return $dup_error;
    }

    $post    = bridge_api_validate_equipment($post_id);
    if (is_wp_error($post)) {
        return $post;
    }
    $before = bridge_format_unit($post_id);
    ...
    bridge_save_unit_fields($post_id, $data);
    $after = bridge_format_unit($post_id);

    $diff    = bridge_diff_unit($before ?: array(), $after ?: array());
    $summary = $before ? bridge_diff_summary($diff) : 'updated unit';
    $rid     = bridge_request_id($request);
    bridge_log_ledger($post_id, 'update', $summary, array('diff' => $diff, 'before' => $before, 'after' => $after), $rid);

    return rest_ensure_response($after);
}

function bridge_api_soft_delete(WP_REST_Request $request) {
    $post_id = intval($request->get_param('id'));
    ...
    update_post_meta($post_id, '_bridge_deleted_at', current_time('c'));
    wp_trash_post($post_id);
    $rid = bridge_request_id($request);
    bridge_log_ledger($post_id, 'delete', 'soft delete', array('deleted_at' => current_time('mysql')), $rid);
...
function bridge_api_restore_unit(WP_REST_Request $request) {
    $post_id = intval($request->get_param('id'));
    if (!current_user_can('edit_post', $post_id)) {
    ...
    delete_post_meta($post_id, '_bridge_deleted_at');
    $rid = bridge_request_id($request);
    bridge_log_ledger($post_id, 'restore', 'restore unit', array(), $rid);
    
    if (function_exists('bridge_purge_cache')) {
        bridge_purge_cache($post_id);
    }
    
    return rest_ensure_response(bridge_format_unit($post_id));
}

function bridge_api_set_marketplace_posted(WP_REST_Request $request) {
    $post_id = intval($request->get_param('id'));
    if (!current_user_can('edit_post', $post_id)) {
        return new WP_Error('forbidden', 'You are not allowed to edit this unit.', array('status' => 403));
    }
    ...
    if ($posted) {
        ...
        update_post_meta($post_id, '_bridge_marketplace_posted_date', current_time('c'));
    } else {
        update_post_meta($post_id, 'marketplace_posted', '0');
        delete_post_meta($post_id, '_bridge_marketplace_posted_date');
    }
...
function bridge_api_permanent_delete(WP_REST_Request $request) {
    $post_id = intval($request->get_param('id'));
    if (!current_user_can('delete_post', $post_id)) {
        return new WP_Error('forbidden', 'You are not allowed to delete this unit.', array('status' => 403));
    }
    ...
    $rid = bridge_request_id($request);
    bridge_log_ledger($post_id, 'permanent_delete', 'permanent delete', array(), $rid);
...
function bridge_api_upload_media(WP_REST_Request $request) {
...
function bridge_api_get_ledger(WP_REST_Request $request) {
    global $wpdb;
    $post_id = intval($request->get_param('id'));
    $post    = bridge_api_validate_equipment($post_id);
    if (is_wp_error($post)) {
...
function bridge_api_get_sessions(WP_REST_Request $request): WP_REST_Response {
    global $wpdb;
    ...
    if ($active_sessions) {
        foreach ($active_sessions as $sess) {
            ...
            if (empty($sess->session_token)) {
                ...
                $data = get_transient('bridge_mobile_token_' . $sess->session_token);
                ...
                if (!$stored_user_id) {
                    ...
                    if ($now - $last_act > 1800) {
                        $is_valid = false;
                        $reason   = 'timeout';
                        delete_transient('bridge_mobile_token_' . $sess->session_token);
                    }
                }
            } else {
    ...
    foreach ($items as &$row) {
        ...
        $row['initials']     = $u ? bridge_user_initials($u) : '';
    }
...
function bridge_api_get_global_ledger(WP_REST_Request $request): WP_REST_Response {
    global $wpdb;
    $table = $wpdb->prefix . 'bridge_inventory_ledger';
 ...
 */
function bridge_api_bootstrap(): WP_REST_Response {
    $user = wp_get_current_user();

    // ── Inventory (same logic as bridge_api_get_inventory, unpaginated) ──
    $args = array(
        'post_type'      => 'equipment',
        'post_status'    => bridge_is_admin_request() ? array('publish', 'draft') : 'publish',
    ...
    $context = bridge_is_admin_request() ? 'edit' : 'public';
    $inventory = array_map(function (WP_Post $p) use ($context): array {
        return bridge_format_unit($p->ID, $context);
    }, $query->posts);
    ...
    // ── Taxonomy lists ──
    $raw_brands       = get_option('bridge_brands');
    $brands           = (is_array($raw_brands) && !empty($raw_brands)) ? $raw_brands : (function_exists('bridge_default_brands') ? bridge_default_brands() : array());
    $raw_categories   = get_option('bridge_categories');
    $categories       = (is_array($raw_categories) && !empty($raw_categories)) ? $raw_categories : bridge_default_categories();
    $subcategories    = get_option('bridge_subcategories', array());
    $sub_subcategories = get_option('bridge_sub_subcategories', array());
    $category_tree     = get_option('bridge_category_tree', array());
...
function bridge_api_logout() {
    if (!is_user_logged_in()) {
        return new WP_Error('unauthorized', 'Not logged in', array('status' => 401));
    }

    // wp_logout() fires the wp_logout action, which triggers bridge_record_logout
    // via add_action('wp_logout', 'bridge_record_logout') in bridge-os-plugin-v23.php.
    // Do NOT call bridge_record_logout() directly here to avoid double-execution.
    wp_logout();
...
function _bridge_sanitize_settings_data(array $params, array $defaults): array {
    $sanitized = array();
            ...
            if (is_array($params[$key])) {
                foreach ($params[$key] as $job) {
                    ...
                    $jobs[] = array(
                        ...
                        'job_location'    => sanitize_text_field($job['job_location'] ?? 'Your City, ST'),
...
function bridge_api_get_settings(): WP_REST_Response {
    $settings = get_option('bridge_theme_settings', array());
    $defaults = bridge_backend_get_settings_defaults();
...
function bridge_api_save_settings(WP_REST_Request $request) {
    $params    = $request->get_json_params();
    if (!is_array($params)) {
        return new WP_Error('invalid_data', 'Invalid settings data', array('status' => 400));
    }

    $sanitized = _bridge_sanitize_settings_data($params, bridge_backend_get_settings_defaults());
    update_option('bridge_theme_settings', $sanitized);
    return rest_ensure_response(array('success' => true, 'settings' => $sanitized));
}

function bridge_api_save_preview_settings(WP_REST_Request $request) {
    $params    = $request->get_json_params();
    if (!is_array($params)) {
        return new WP_Error('invalid_data', 'Invalid settings data', array('status' => 400));
    }
    ...
    $sanitized = _bridge_sanitize_settings_data($params, bridge_backend_get_settings_defaults());
    update_option('bridge_theme_settings_preview', $sanitized);
    return rest_ensure_response(array('success' => true, 'settings' => $sanitized));
}
...
function bridge_api_session_nonce() {
...
function bridge_api_generate_mobile_token() {
    ...
    $cooldown_key = 'bridge_token_cooldown_' . $user_id;
    ...
    $active_key    = 'bridge_active_tokens_' . $user_id;
    ...
    $active_tokens = array_values(array_filter($active_tokens, function ($t) {
        return (bool) get_transient('bridge_mobile_token_' . $t);
    }));
    if (count($active_tokens) >= 3) {
        $oldest = array_shift($active_tokens);
        delete_transient('bridge_mobile_token_' . $oldest);
    }
    ...
    set_transient('bridge_mobile_token_' . $token, array('user_id' => $user_id, 'created_at' => time()), 1800);
    set_transient('bridge_handoff_' . $nonce, $token, 120); // nonce expires in 2 minutes
    ...
    bridge_set_mobile_token_cookie($token);
...
function bridge_set_mobile_token_cookie(string $token): void {
    if (is_ssl()) {
        setcookie('bridge_mobile_token', $token, array(
...
function bridge_clear_mobile_token_cookie(): void {
    setcookie('bridge_mobile_token', '', array(
        'expires'  => time() - 3600,
        'path'     => '/',
        'secure'   => is_ssl(),
        'httponly'  => true,
...
// re-mint a token via the cookie->auto-token path in bridge-facebook-pwa.php,
// and returns a short-lived mobile token for the React app's REST calls now.
function bridge_api_mobile_login(WP_REST_Request $request) {
    // Rate limit: 5 attempts / 15 min per client IP (brute-force guard). [tunable]
    $ip       = bridge_login_client_ip();
    $rl_key   = 'bridge_login_rl_' . substr(md5($ip), 0, 20);
    ...
    return rest_ensure_response(array(
        'token' => bridge_mint_mobile_token($user->ID),
        'nonce' => wp_create_nonce('wp_rest'),
...
// bridge_api_generate_mobile_token (which also issues a handoff nonce/URL).
function bridge_mint_mobile_token(int $user_id): string {
    $active_key    = 'bridge_active_tokens_' . $user_id;
    $active_tokens = get_transient($active_key) ?: array();
...
function bridge_login_client_ip(): string {
    $ip = $_SERVER['REMOTE_ADDR'] ?? '';
    return filter_var($ip, FILTER_VALIDATE_IP) ? $ip : '0.0.0.0';
}

// NOTE: Mobile auth filters (determine_current_user, rest_authentication_errors)
// and the session activity tracker (init hook) have been moved to bridge-os-plugin-v23.php
...
function bridge_api_get_meta_sync_logs(): WP_REST_Response {
    $logs = get_option('bridge_meta_sync_logs', array());
    if (!is_array($logs)) {
        $logs = array();
    }
    ...
    if (empty($logs)) {
        ...
        update_option('bridge_meta_sync_logs', $logs);
    }
    return rest_ensure_response($logs);
}

function bridge_api_get_meta_sync_health(): WP_REST_Response {
    $cached = get_transient('bridge_meta_sync_health');
    if ($cached !== false) {
    ...
    if ($total === 0) {
        ...
        set_transient('bridge_meta_sync_health', $result, 300);
    ...
    set_transient('bridge_meta_sync_health', $result, 300); // cache for 5 minutes

    return rest_ensure_response($result);
}

function bridge_api_get_unit(WP_REST_Request $request): WP_REST_Response|WP_Error {
    $id = intval($request->get_param('id'));
    $post = bridge_api_validate_equipment($id);
    if (is_wp_error($post)) {
        return $post;
    }
    $unit = bridge_format_unit($id);
    return rest_ensure_response($unit);
}
